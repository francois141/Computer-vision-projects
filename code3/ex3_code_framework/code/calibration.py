...
def main():
  ...
  p2d = points2D
  p3d = points3D
  points2D, T2D = NormalizePoints2D(points2D,image_size)
  points3D, T3D = NormalizePoints3D(points3D)

  normalized_points2D = MakeHomogeneous(points2D,ax=1)
  normalized_points3D = MakeHomogeneous(points3D,ax=1)
  ...
  PlotProjectedPoints(p3d, p2d, K, R, t, image_size, ax2d)
