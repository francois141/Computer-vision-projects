...
def UpdateReconstructionState(new_points3D, corrs, points3D, images):
  ...
  for im_name in corrs:
    num_corrs = len(corrs[im_name])
    images[im_name].Add3DCorrs(corrs[im_name], list(range(idx_offset+ points3D.shape[0], idx_offset + points3D.shape[0] + num_corrs, 1)))
    if num_corrs < new_points3D.shape[0]:
      idx_offset += num_corrs
