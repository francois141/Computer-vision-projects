...
def TriangulatePoints(K, im1, im2, matches):
  ...
  values1 = (P1 @ MakeHomogeneous(points3D,ax=1).transpose()).transpose()[:,2]
  values2 = (P2 @ MakeHomogeneous(points3D,ax=1).transpose()).transpose()[:,2]
  ...
  for i in range(values1.shape[0]):
    if values1[i] >= 0 and values2[i] >= 0:
      points3D_filtered.append(points3D[i])
      im1_corrs_filtered.append(im1_corrs[i])
      im2_corrs_filtered.append(im2_corrs[i])

  return np.array(points3D_filtered), np.array(im1_corrs_filtered), np.array(im2_corrs_filtered)
...
def TriangulateImage(K, image_name, images, registered_images, matches):
  ...
  for image_other_name in registered_images:

    # Triangulate
    image_other = images[image_other_name]
    points3D_new, im1_corr_new,im2_corr_new = TriangulatePoints(K,image,image_other,GetPairMatches(image_name,image_other_name,matches))

    # Get current length and update points
    length = points3D.shape[0]
    if points3D_new.shape[0] != 0:
      points3D = np.append(points3D,points3D_new,axis=0)

    # Update the data structure
    # Update image 1
    if image in corrs:
      corrs[image_name] = np.append(corrs[image_name],im1_corr_new + length) # add points 3D size  
    else:
      corrs[image_name] = im1_corr_new # current length is 0 so add nothing
    # Update image 2
    corrs[image_other_name] = im2_corr_new + length  # add points 3D Size in the index
