...
def main():
  ...
  for i in range(len(possible_relative_poses)):
    ...
    e_im2.SetPose(np.eye(3),np.zeros(shape=(3)))
    points,_,_ = TriangulatePoints(K,e_im1,e_im2,e_matches)
    if maxPoints < points.shape[0]:
      maxPoints = points.shape[0]
      bestPose = i
 
  e_im1.SetPose(possible_relative_poses[bestPose][0],possible_relative_poses[bestPose][1])
  e_im2.SetPose(np.eye(3),np.zeros(shape=(3)))
  ...
  images[e_im1_name] = e_im1
  images[e_im2_name] = e_im2
